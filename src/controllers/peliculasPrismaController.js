...
const listarPeliculas = async (req, res, next) => {
  try {
    ...
    // Usar transacción para obtener datos y conteo en una sola operación
...
const crearPelicula = async (req, res, next) => {
  try {
    ...
    // Usar transacción para manejar la creación/actualización de director y género
    const pelicula = await prisma.$transaction(async (tx) => {
      let directorId = null
      if (director) {
        // upsert: crea si no existe, o devuelve el existente
      ...
      let generoId = null
      if (genero) {
        // Buscar género por slug o nombre (insensible a mayúsculas)
      ...
      // Crear la película con las relaciones
...
const eliminarPelicula = async (req, res, next) => {
  try {
    ...
    // Eliminar película (Prisma manejará las relaciones con Cascade)
